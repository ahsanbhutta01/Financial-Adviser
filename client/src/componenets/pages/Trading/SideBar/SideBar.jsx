...
const SideBar = ({ expand, setExpand, userName, handleLogout, onLabelClick }) => {
   ...
   return (
      <>
         <div className={`flex relative flex-col justify-between bg-[#212327] pt-2 md:pt-7 transition-all z-50 max-md:absolute max-md:h-screen  ${expand ? 'p-4 w-64 md:w-80' : ' w-0 max-md:overflow-hidden'}`}>
            <div className='ml-1'>
               <button 
                  className={`mt-15 flex items-center text-xl justify-center cursor-pointer ${expand ? 'bg-[#5BB0FF] text-white hover:opacity-90 rounded-xl gap-2 p-1 px-3 w-max' : 'group relative size-5 mx-auto rounded-lg'}`}
                  onClick={() => {
                     onLabelClick(null)
               ...
               >
                  ...
                  <div className='absolute w-max -top-2 left-15 opacity-0 group-hover:opacity-100 transition bg-black text-white text-sm px-3 py-2 rounded-lg shadow-lg pointer-events-none'>
                     New Chat
                  </div>
            ...
            <div className={`md:hidden flex items-center gap-1 ${expand ? 'hover:bg-white/10 rounded-lg' : 'justify-center w-full'} p-1 cursor-pointer`} onClick={() => setSetting(!setting)}>
               <img
                  ...
                  className='size-10'
               />
               {expand && <span className='text-white'>Hi, {userName}</span>}
            </div>

            {/* For mobile screen setting */}
            {setting && (
               <div className="transition-all duration-300 bg-white w-50 text-center rounded-2xl md:hidden absolute bottom-16 z-51">
